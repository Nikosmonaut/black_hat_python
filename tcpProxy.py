...
def receive_from(connection):
    ...
    # target, this may neew to be adjusted
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        if len(local_buffer):
            ...
            # if not more data on either side, close the connections
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    print("[*] Listening to listen on %s:%d" % (local_host, local_port))
...
def main():
    # no fancy comman-line parsing here
    if len(sys.argv[1:]) != 5:
    ...
    # this tells our proxy to conenct and receive data
